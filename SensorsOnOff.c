...
//EPS- MOSFET control for mosfet bridge 
void LEDL_SWITCH_TO_EPS(void){
P7DIR|=BIT2;
P7OUT&=~BIT2;
}
//mosfet control for fuse
...
*/
void SetUpLEDs(void){
P7DIR |= BIT0|BIT1|BIT2|BIT3|BIT5|BIT6|BIT7;//LIGHT UP LED'S AS OUTPUTS 
P7OUT &= ~(BIT0|BIT1|BIT2|BIT3|BIT5|BIT6|BIT7);
}
...
//LED_1 is used for finding a free address in memory, once found it turns off
void LED_1_ON(void){
P7OUT|=BIT1;
}
void LED_1_OFF(void){
...
P7OUT &= ~0xFF;//turn all LED's off, use in a reset
}